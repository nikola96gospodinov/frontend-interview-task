...
1. Given more time, what other changes you would have liked to make? - It would have been mainly stylistic changes described in more detail in the next point
2. What UX or design improvements or alterations might you suggest? These can be to existing components or completely new ideas. - Everything feels a bit scattered at the moment. The first section could easily be split into two columns without causing too much noise. Also since there is some color introduced below it might be a good idea to put the estimated value in a container that is either green or red depending on the price movement and the percentage change next to the price. The reason for the container is that this is the most important information on the page and we want the user to look at it first. The use of simple icons or labels could be used further below to make things more clear about what the different items represent rather than just having the value and letting it to the user to determine what the info means.
